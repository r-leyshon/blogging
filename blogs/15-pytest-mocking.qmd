...
* **Step 3**
    * When invoking the function, be sure to reference the function in the same
    way as it was monkeypatched. Aliases can also be used if preferable
    (eg `import example_pkg.only_joking as jk`).
...
* **unstub**
    ...
    * `mockito` allows you to implicitly `unstub()` by using a context manager
    like `with`.
