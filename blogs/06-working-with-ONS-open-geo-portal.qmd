...
def request_to_gdf(url:str, query_params:dict) -> gpd.GeoDataFrame:
    ...
    requests.exceptions.RequestException
        The response was not ok or was ok but returned content with an error -
        weird ArcGIS quirk.
    """
    ...
    if response.ok:
        ...
        if "error" in content.keys():
            # arcGIS strange approach to serving HTTP errors
            error = content["error"]
            raise requests.exceptions.RequestException(
                f"HTTP Code: {error['code']}, Status: {error['details']}"
            )
        else:
            # good response. This approach will only work for geoJSON response.
            return (
