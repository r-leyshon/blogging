...
with the Python `requests` package or ArcGIS REST API @ArcGIS. 
...
2. Using the ribbon menu at the top of the page, navigate to Boundaries {{< fa arrow-right >}} Census Boundaries {{< fa arrow-right >}} Lower Super Output Areas {{< fa arrow-right >}} 2021 Boundaries. 
...
5. Now we need to add a new parameter to our `params` dictionary, with the key
...
adding a retry strategy to the requests. @ZenRows 
