...
2. The inverted and scaled median travel time, assigned to
`med_tts["inverted_med_tt"]`.
...
```{python}
# Tidy up
tmp.cleanup() # only needed if tmp director used
...
map visuals, pan out and check that an incorrect coordinate specification has
...
could be informed by locations where people would wish to commute, such as
retail, businesses, tourist attractions and residential areas. OpenStreetMap
files are a rich source of location data, which can be extracted with libraries
such as [pyosmium](https://osmcode.org/pyosmium/). 
