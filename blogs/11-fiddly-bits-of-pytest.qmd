...
column values out and casting to lists. The `pandas` package has it's own
...
a test class that will house all tests designed to test the functionality of 
`serve_food()`. Within that class let's define our first test that simply
checks that the value of the `has_munchies` column has been updated as we would
expect after using the `serve_food()` function.
...
Now we can update the test class to include a new test that checks the state of
the fixtures. At this point, we will start to see some differences due to the
scoping of the fixtures. The new `test_expected_states_within_same_class()`
will assert that the changes to the fixtures brought about in the previous test
...
the original values. The function-scoped fixture is executed again as
`test_expected_states_within_same_class` is executed, ensuring a 'fresh',
unchanged version of the fixture DataFrame is provided. 
...
exists within a new class `TestSomeOtherTestClass`, the code for
...
While the persistence of fixtures may be useful for end to end tests or
similar, this approach reduces isolation in the test suite. Be aware that this
may introduce a bit of friction to your pytest development process. For
example, it can be commonplace to develop a new test and to check that it
passes by invoking pytest with the keyword `-k` flag to run that single test
only. This approach useful if you have a costly test suite and you just want to
examine changes in a single unit.

However, with the current state of the test module `test_feed_characters.py`,
executing the entire test module by running
`pytest ./tests/test_feed_characters.py` will pass while running
...
test class `TestServeFood`. Tests in `TestSomeOtherTestClass` rely on changes
elswehere in your test suite and by definition are no longer unit tests. For
those developers who work with
[pytest-randomly](https://pypi.org/project/pytest-randomly/) to help sniff out
poorly-isolated tests, this approach could cause a bit of a headache.
...
executed seperately, though the implementation of this is beyond the scope of
this article. 
...
"empty". Now to test this this new src module, we create a new test module. We
will run assertions of the `fave_food` columns against the differently-scoped
fixtures. 
...
only. All other fixture scopes will go through setup once more on execution of
the second test module.

Note that this arrangement is highly dependent on the order of which the test
modules are collected. `pytest` collects tests in alphabetical ordering by
default, and as such `test_update_food.py` can be expected to be executed after
`test_feed_characters.py`. As in the case of
[the previous section](#balancing-isolation--persistence), this test module
is highly dependent upon the order of the pytest execution. This makes the
tests less portable and means that running the test module  with
`pytest tests/test_update_food.py` in isolation would fail. I would once more
suggest using
[`pytest` marks](https://docs.pytest.org/en/latest/how-to/mark.html) to group
these types of tests and execute them seperately to the rest of the test suite.
...
The reason for `ScopeMismatch` is straight forward. Fixture scopes have a
...
hierarchy rule, so updating `_fix_a` to use a class cope or greater would
...
If you spot an error with this article, feel free to
